...
def main():
    ...
    parser.add_argument(
        "--output_folder",
        help="batchsize",
        default='G:\OneDrive\code\python\kaggle/fashion/inferences',
    )
    parser.add_argument(
        "opts",
    ...
    _ = checkpointer.load("G:/model_0047500.pth", use_latest=False)
    ...
    data_loader_val = makeFashionDataLoader(cfg,datasets[0],test=True)
    
    ## 由于只有一个dataset，所以可以去掉循环
    # for output_folder, dataset_name, data_loader_val in zip(output_folder,  data_loader_val):
    inference(
        model,
        data_loader_val,
