...
from maskrcnn_benchmark.engine import visual_proposal,F1_scorer
...
def do_train(
    ...
    for iteration, (images, targets, _) in tqdm(enumerate(data_loader, start_iter),desc="Training process"):
        ...
        loss_dict,proposals = model(images, targets)
        
        
        """
        可视化RPN Proposal
        """
        # visual_proposal(image,proposals)
        ...
        ## 暂时还用不到推断的数据来查看训练过程，所以先注释掉inference节约时间
        if data_loader_val is not None and test_period > 0 and iteration % test_period == 0:
            ...
            #     makeFashionDataLoader(cfg,dataset_val),
